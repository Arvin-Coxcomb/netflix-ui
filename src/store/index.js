...
const getRawData = async(api, genres, paging) =>{
    ...
    for(let i=1; moviesArray.length <60 && i<10; i++ ){
        const {data : {results}} = await axios.get(`${api}${paging ? `&page=${i}` : ""}`).catch((err)=>console.log(err))
        //console.log(results);
...
export const removeFromUserLikedMovies = createAsyncThunk(
    "Netflix/removeFromLikedMovies",
    async({email,movieId}) => {
...
const NetflixSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(removeFromUserLikedMovies.fulfilled, (state, action) => {
            state.movies = action.payload;
          });
    },
});
